        ...
        //상품 선택시 단가 입력 및 체크 상태 변경
      	...
      	function makeData(){
			...
			//체크한 행만 전송할 데이터 만들기.
			  ...
			  //console.log(checkbox); // <input type="checkbox" name="selId" checked="checked"> dom 형식
				console.log( $(checkbox) ); // jquery 객체로 나옴
			  //console.log($(this)); 
      		...
      		// ajax 호출
      		$.ajax({
      			url:'/exam/insertGood', //http://localhost/exam/xxxx
      			type:'post', // method
      			contentType: "application/json", // JSON 형태로 보내고싶은 경우에 반드시 지정해야 함. queryString의 경우 ?no=100&name=scott JSON -> {no:100, name:"scott"}
      			data:JSON.stringify(list) // list는 현재 js 객체. 이걸 string 으로 바꿔서 보내야 함.
      			                          //  JSON.stringify({customer:"hong", ord_goods:"G10"})
